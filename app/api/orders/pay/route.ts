...
export async function PATCH(request: NextRequest) {
  ...
  const [{ error: mainError }, { error: subError }] = await Promise.all([
    supabase.from("PARAGELIA").update({ status: "paid" }).eq("id", orderId),
    supabase.from("PARAGELIA").update({ status: "paid" }).eq("parent_order", orderId),
  ])

  if (mainError || subError) {
    console.error("[PAY ERROR]", mainError ?? subError)
